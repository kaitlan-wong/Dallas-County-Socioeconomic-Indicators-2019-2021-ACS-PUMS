---
title: "DEI ACS PUMS Data Checks - Indicator 30: Utility Expenses - 2017 Final Version"
...
-   ACS PUMS Data Documentation: <https://www.census.gov/programs-surveys/acs/microdata/documentation.2017.html#list-tab-1370939201>

-   2017 ACS Data Dictionary: <https://www2.census.gov/programs-surveys/acs/tech_docs/pums/data_dict/PUMS_Data_Dictionary_2017.pdf>
...
u17$ADJHSG  # this is already in decimal format, so no need to divide by 1e+06 (unlike 2017 pums dataset)

u17$ADJINC  # this is already in decimal format, so no need to divide by 1e+06
```
...
has_na <- anyNA(pumas17)

if (has_na) {
  # If there are NAs, remove rows with NAs
  pumas17 <- pumas17[complete.cases(pumas17), ]
...
pumas17$ADJHSG <- as.numeric(pumas17$ADJHSG)
pumas17$GASP <- as.numeric(pumas17$GASP)
pumas17$ELEP <- as.numeric(pumas17$ELEP)
pumas17$FULP <- as.numeric(pumas17$FULP)
pumas17$WATP <- as.numeric(pumas17$WATP)
...
pumas17<- pumas17 %>% mutate(gas = adj_gasp * 12) %>%
...
# covert variables to numeric
pumas17$ADJINC <- as.numeric(pumas17$ADJINC)
pumas17$HINCP <- as.numeric(pumas17$HINCP)

# Remove households with income less than $0
pumas17 <- pumas17 %>% filter(HINCP >=0)
...
median_percent_income_utilities_by_race <- pumas17 %>%
  group_by(race_eth) %>%
...
write_xlsx(list("med_inc" = median_income_by_race, "med_util" = median_costs_by_race, "percent_inc" = median_percent_income_utilities_by_race), "30_Utility Expenses_2017.xlsx")
```
