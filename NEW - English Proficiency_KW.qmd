---
title: "DEI ACS PUMS Data Checks - NEW Potential Indicator: English Proficiency"
author: "Kaitlan Wong"
format: html
editor: visual
---
...
data21 <- pums21 %>%
  ...
  # recode English proficiency variable
  mutate(eng_prof = case_when(
    ENG %in% c(2:4) ~ "Does not speak English at all or very well",
    ENG %in% c(1) ~ "Speaks English very well",
    TRUE ~ NA_character_)
   )
```
...
```{r}
# Calculate # and % of people who do no seak Engish very well by race/ethnicity
summary21 <- data21 %>%
  group_by(race_eth) %>%
  summarize(
    count = sum(PWGTP[eng_prof == "Does not speak English at all or very well"], na.rm = TRUE),
    prop = sum(PWGTP[eng_prof == "Does not speak English at all or very well"], na.rm = TRUE) / sum(WGTP)
  )

summary21


# Calculate # and % of people who do no seak Engish very well
summary_all21 <- data21 %>%
  summarize(
    count = sum(PWGTP[eng_prof == "Does not speak English at all or very well"], na.rm = TRUE),
    prop = sum(PWGTP[eng_prof == "Does not speak English at all or very well"], na.rm = TRUE) / sum(PWGTP, na.rm = TRUE)
  )
...
summary19 <- data19 %>%
  ...
  summarize(
    count = sum(PWGTP[eng_prof == "Does not speak English at all or very well"], na.rm = TRUE),
    prop = sum(PWGTP[eng_prof == "Does not speak English at all or very well"], na.rm = TRUE) / sum(PWGTP)
  )
...
write_xlsx(list("2021" = summary21, "2019" = summary19), "NEW_English Proficiency_KW.xlsx")
```
