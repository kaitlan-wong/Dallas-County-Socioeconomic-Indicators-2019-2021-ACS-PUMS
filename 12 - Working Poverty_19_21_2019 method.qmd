---
title: "DEI ACS PUMS Data Checks - Indicator 12: Working Poverty - 2019-2021 (2019 report method)"
author: "Kaitlan Wong"
format: html
editor: visual
---
...
# Get ACS data
pums21 <- get_pums(
  variables = c("AGEP", "RAC1P", "HISP", "PUMA", "POVPIP", "ESR", "WKHP", "WKWN"),
  state = "TX",
  survey = "acs1",
  year = 2021,
  recode = TRUE,
...
data21 <- pums21 %>%
  filter(PUMA %in% c("02304", "02305", "02306", "02307", "02309", "02310", "02311", "02312", "02313", "02314", "02315", "02316", "02319", "01901", "02001"),
         # filter for working-age aduls
         AGEP >= 25 & AGEP <= 64,
         # filter for adults that worked in past 12 months
         WKWN %in% c(50:52), # this is the 2019 method, rather than using ESR. We are filtering for adults that worked 50-52 weeks in the apst year.
         # usually work 30+ hours/week
         WKHP >= 30) %>% # adults who usually worked 30+ hours a week
  ...
  mutate(pov = case_when(
    POVPIP %in% c(0:200) ~ "At or below 200% poverty",
    POVPIP < 0 ~ "NA",
    TRUE ~ "Above 200% poverty")
   )
```
...
```{r}
# Calculate # and % in poverty by race/ethnicity
summary21 <- data21 %>%
  group_by(race_eth) %>%
  summarize(
    count_pov = sum(PWGTP[pov == "At or below 200% poverty"], na.rm = TRUE),
    prop_pov = sum(PWGTP[pov == "At or below 200% poverty"], na.rm = TRUE) / sum(PWGTP)
  )

summary21


# Calculate total # and % in poverty
...
# Get ACS data
pums19 <- get_pums(
  variables = c("AGEP", "RAC1P", "HISP", "PUMA", "WKHP", "POVPIP", "ESR", "WKWN"),
  state = "TX",
  survey = "acs1",
  year = 2019,
  recode = TRUE,
...
write_xlsx(list("2021" = summary21, "2019" = summary19), "12_Working Poverty_19_21_2019 method.xlsx")
```
