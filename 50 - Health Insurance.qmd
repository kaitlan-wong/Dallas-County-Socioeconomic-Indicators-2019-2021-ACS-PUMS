---
title: "DEI ACS PUMS Data Checks - Indicator 50: Health Insurance"
author: "Kaitlan Wong"
format: html
editor: visual
---
...
# Get ACS data
pums21 <- get_pums(
  variables = c("AGEP", "RAC1P", "HISP", "PUMA", "HICOV"),
  state = "TX",
  survey = "acs1",
  year = 2021,
  recode = TRUE,
...
```{r}
# Calculate # and % of uninsured residents by race/ethnicity
summary21 <- data21 %>%
  group_by(race_eth) %>%
  summarize(
    count = sum(PWGTP[hi == "Uninsured"], na.rm = TRUE),
    prop = sum(PWGTP[hi == "Uninsured"], na.rm = TRUE) / sum(PWGTP)
  )

summary21


# Calculate total # and % of uninsured residents
...
# Calculate # and & of children in poverty by race/ethnicity
summary19 <- data19 %>%
  group_by(race_eth) %>%
  summarize(
    count = sum(PWGTP[hi == "Uninsured"], na.rm = TRUE),
...
# Calculate total # and % of children in poverty
...
write_xlsx(list("2021" = summary21, "2019" = summary19), "50_Health Insurance.xlsx")
```
