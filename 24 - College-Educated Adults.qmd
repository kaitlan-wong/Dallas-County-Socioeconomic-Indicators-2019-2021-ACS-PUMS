---
title: "DEI ACS PUMS Data Checks - Indicator 24: College-Educated Adults"
author: "Kaitlan Wong"
format: html
editor: visual
---
...
# Get ACS data
pums21 <- get_pums(
  variables = c("AGEP", "RAC1P", "HISP", "PUMA", "SCH", "SCHG", "POVPIP", "SCHL"),
  state = "TX",
  survey = "acs1",
  year = 2021,
  recode = TRUE,
...
data21 <- pums21 %>%
  ...
  # recode educational attainment variable to measure Hs diploma attainment
  mutate(degree = case_when(
    SCHL %in% c(21:24) ~ "Bachelor's or higher",
    SCHL == 0 ~ "NA",
    TRUE ~ "No Bachelor's")
   ) 
```
...
```{r}
# Calculate # and % w/ Bachelor's degree by race/ethnicity
summary21 <- data21 %>%
  group_by(race_eth) %>%
  summarize(
    count = sum(PWGTP[degree == "Bachelor's or higher"], na.rm = TRUE),
    prop = sum(PWGTP[degree == "Bachelor's or higher"], na.rm = TRUE) / sum(PWGTP)
  )

summary21


# Calculate total # and % w/ Bachelor's degree
summary_all21 <- data21 %>%
  summarize(
    count = sum(PWGTP[degree == "Bachelor's or higher"], na.rm = TRUE),
    prop = sum(PWGTP[degree == "Bachelor's or higher"], na.rm = TRUE) / sum(PWGTP, na.rm = TRUE)
  )
...
write_xlsx(list("2021" = summary21, "2019" = summary19), "24_College-Educated Adults.xlsx")
```
