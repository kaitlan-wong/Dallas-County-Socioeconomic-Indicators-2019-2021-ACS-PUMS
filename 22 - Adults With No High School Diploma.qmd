---
title: "DEI ACS PUMS Data Checks - Indicator 22: Adults With No High School Diploma"
author: "Kaitlan Wong"
format: html
editor: visual
---
...
data21 <- pums21 %>%
  ...
  mutate(HSdip = case_when(
    SCHL %in% c(16:24) ~ "HS diploma",
    SCHL == 0 ~ "NA",
    TRUE ~ "No HS diploma")
   ) 
```
...
```{r}
# Calculate # and % w/no diploma by race/ethnicity
summary21 <- data21 %>%
  group_by(race_eth) %>%
  summarize(
    count = sum(PWGTP[HSdip == "No HS diploma"], na.rm = TRUE),
    prop = sum(PWGTP[HSdip == "No HS diploma"], na.rm = TRUE) / sum(PWGTP)
  )

summary21


# Calculate total # and % w/no diploma
...
write_xlsx(list("2021" = summary21, "2019" = summary19), "22_Adults With No HS Diploma.xlsx")
```
