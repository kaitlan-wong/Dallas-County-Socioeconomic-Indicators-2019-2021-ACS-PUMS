---
title: "DEI ACS PUMS Data Checks - Indicator 23: High Shcool Graduate Living in Poverty"
author: "Kaitlan Wong"
format: html
editor: visual
---
...
data21 <- pums21 %>%
  ...
  mutate(pov = case_when(
    POVPIP %in% c(0:99) ~ "At or below 100% poverty",
    POVPIP < 0 ~ "NA",
    TRUE ~ "100% or above poverty")
   )
```
...
summary_all21 <- data21 %>%
  summarize(
    count_pov = sum(PWGTP[pov == "At or below 100% poverty"], na.rm = TRUE),
    prop_pov = sum(PWGTP[pov == "At or below 100% poverty"], na.rm = TRUE) / sum(PWGTP, na.rm = TRUE)
  )
...
write_xlsx(list("2021" = summary21, "2019" = summary19), "23_HS Grads Living in Poverty.xlsx")
```
