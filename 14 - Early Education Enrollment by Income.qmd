---
title: "DEI ACS PUMS Data Checks - Indicator 14: Early Education Enrollment by Income"
author: "Kaitlan Wong"
format: html
editor: visual
---
...
data21 <- pums21 %>%
  ...
  mutate(preK = case_when(
    SCH %in% c(2:3) ~ "Attends preK",
    SCH == 0 ~ "NA",
    TRUE ~ "Not in preK")
   ) %>%
  mutate(income = case_when(
    POVPIP >= 185 ~ "High",
    POVPIP %in% c(101:185) ~ "Middle",
    POVPIP %in% c(0:100) ~ "Low",
...
write_xlsx(list("2021" = summary21, "2019" = summary19), "13_Early Education Enrollment by Race.xlsx")
```
