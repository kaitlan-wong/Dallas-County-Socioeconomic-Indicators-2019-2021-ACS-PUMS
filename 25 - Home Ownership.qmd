---
title: "DEI ACS PUMS Data Checks - Indicator 25: Home Ownership"
author: "Kaitlan Wong"
format: html
editor: visual
---
...
# Get ACS data
pums21 <- get_pums(
  variables = c("AGEP", "RAC1P", "HISP", "PUMA", "TEN"),
  state = "TX",
  survey = "acs1",
  year = 2021,
  recode = TRUE,
...
data21 <- pums21 %>%
  ...
  # recode home ownership variable
  mutate(own = case_when(
    TEN %in% c(1:2) ~ "Owns a home",
    TEN == 0 ~ "NA",
    TRUE ~ "Does not own a home")
   )
```
...
```{r}
# Calculate # and % of households who own their home by race/ethnicity of householder
summary21 <- data21 %>%
  group_by(race_eth) %>%
  summarize(
    count = sum(WGTP[own == "Owns a home"], na.rm = TRUE),
    prop = sum(WGTP[own == "Owns a home"], na.rm = TRUE) / sum(WGTP)
  )

summary21


# Calculate total # and % of households who own their home
summary_all21 <- data21 %>%
  summarize(
    count = sum(WGTP[own == "Owns a home"], na.rm = TRUE),
    prop = sum(WGTP[own == "Owns a home"], na.rm = TRUE) / sum(WGTP, na.rm = TRUE)
  )
...
write_xlsx(list("2021" = summary21, "2019" = summary19), "25_Home Ownership.xlsx")
```
